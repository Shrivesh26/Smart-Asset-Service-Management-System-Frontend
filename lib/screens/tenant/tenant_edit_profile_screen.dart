...
extension BusinessTypeX on BusinessType {
  String get apiValue {
    switch (this) {
      case BusinessType.homeServices:        return 'home services';
      case BusinessType.cleaningServices:    return 'cleaning services';
      case BusinessType.repairMaintenance:   return 'repair & maintenance';
      case BusinessType.salon:              return 'salon';
      case BusinessType.fitness:            return 'fitness';
      case BusinessType.consulting:         return 'consulting';
  ...
  IconData get icon {
    switch (this) {
      case BusinessType.homeServices:        return Icons.home_repair_service_outlined;
      case BusinessType.cleaningServices:    return Icons.cleaning_services_outlined;
      case BusinessType.repairMaintenance:   return Icons.build_outlined;
      case BusinessType.salon:              return Icons.content_cut_outlined;
      case BusinessType.fitness:            return Icons.fitness_center_outlined;
      case BusinessType.consulting:         return Icons.business_center_outlined;
  ...
  static BusinessType? fromApi(String? value) {
    ...
    try {
      return BusinessType.values
          .firstWhere((e) => e.apiValue.toLowerCase() == value.toLowerCase());
    } catch (_) {
      return null;
    }
  }
...
class _TenantEditProfileScreenState extends State<TenantEditProfileScreen> {
  ...
  // _previewBytes  → MemoryImage for local preview (no _Namespace error)
  // _pickedXFile   → XFile kept to extract filename for upload
  XFile?     _pickedXFile;
  ...
  // ── Pick image ──────────────────────────────────────────────────────
  // Uses XFile.readAsBytes() which works on both mobile AND web.
  // Never touches dart:io File — that crashes on web with _Namespace.
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      // ✅ Pass bytes + filename — no dart:io File involved
      if (_previewBytes != null && _pickedXFile != null) {
        final imageName = _pickedXFile!.name.isNotEmpty
            ? _pickedXFile!.name
            : 'profile.jpg';
        final imageUrl =
            await auth.uploadTenantProfileImage(_previewBytes!, imageName);
      ...
      // Clear local preview — server image is now source of truth
      _pickedXFile  = null;
      _previewBytes = null;
                ...
                _buildSection(
                  ...
                  children: [
                    _field(label: 'Street Address', controller: _addressCtrl,
                        icon: Icons.map_outlined, disabled: false),
                    _field(label: 'City', controller: _cityCtrl,
                        icon: Icons.location_city_outlined, disabled: false),
                    _field(label: 'State', controller: _stateCtrl,
                        icon: Icons.flag_outlined, disabled: false),
                    _field(
                      label: 'Pincode',
                      controller: _pinCtrl,
  ...
  Widget _field({
    ...
    return Padding(
      ...
      child: TextFormField(
        ...
        validator: validator,
        enabled: !disabled,
        style: TextStyle(color: disabled ? _txtS : _txtP, fontSize: 14),
